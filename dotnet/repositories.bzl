...
def _dotnet_repo_impl(repository_ctx):
    ...
    use_src_dirs = bool(
        repository_ctx.getenv("DOTNET_TOOLCHAIN_USE_SOURCE_DIRECTORIES")
    ) and repository_ctx.attr.platform.endswith("linux-gnu")
    # TODO: support macOS/windows/etc (source directory paths need to be adjusted?)
...
dotnet_toolchain(
    ...
    compiler_executables_are_covered_by_source_directories_in_runfiles = {use_src_dirs},
)
"""

    # NOTE: we're changing what's included in the `BUILD.bazel` file so that we
    # don't have to pay the cost of globbing if using source directories.

    # TODO: handle `fsc_binary` properly here!! (untested, probably broken)
    #
...
filegroup(
    name = "csc_binary",
    srcs = ["sdk/{sdk_version}/Roslyn/bincore/csc.dll"],
    data = ["sdk/{sdk_version}/Roslyn/bincore"],
    visibility = ["//visibility:public"],
)
...
filegroup(
    ...
    data = [
        "host/fxr/{runtime_version}/libhostfxr.so",
...
filegroup(
    name = "fsc_binary",
    srcs = [
        "sdk/{sdk_version}/FSharp/fsc.dll",
    ],
    data = [
        "host/fxr/{runtime_version}/libhostfxr.so",
        "sdk/{sdk_version}/FSharp",
    ],
    visibility = ["//visibility:public"],
)
"""
...
dotnet_repositories = repository_rule(
    ...
    doc = _DOC,
    attrs = _ATTRS,
    environ = ["DOTNET_TOOLCHAIN_USE_SOURCE_DIRECTORIES"]
)
